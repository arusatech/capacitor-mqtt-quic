...
/* Arch-specific build flag and SP math implementation — selected at compile
 * time so this single header works for any slice (device arm64, simulator
 * arm64, simulator x86_64). The chosen WOLFSSL_*_BUILD must match the SP
 * implementation, and only the SP routines for the active arch are used. */
