...
- ⏳ Build WolfSSL for iOS (or use pre-built; plugin uses WolfSSL)
...
- ✅ Build scripts (`build-ngtcp2.sh`, `build-wolfssl.sh`; optional `build-openssl.sh`)
- ✅ Build script (`build-nghttp3.sh`)
- ✅ Build documentation (`NGTCP2_BUILD_INSTRUCTIONS.md`)
...
- ⏳ Build WolfSSL (wolfssl-android) for Android (or use pre-built; OpenSSL/QuicTLS optional)
...
- `android/build-wolfssl.sh` (default), `android/build-openssl.sh` (optional)
