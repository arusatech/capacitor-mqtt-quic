...
# Find WolfSSL (default; or OpenSSL if using QuicTLS)
# Plugin CMake uses wolfssl-android paths – see android/src/main/cpp/CMakeLists.txt
find_package(OpenSSL REQUIRED)  # or WolfSSL as in plugin
...
# Default: WolfSSL (installs to android/install/wolfssl-android/<abi>/)
./build-wolfssl.sh --abi arm64-v8a --platform android-21

# Build ngtcp2 with WolfSSL (installs to android/install/ngtcp2-android/<abi>/)
./build-ngtcp2.sh --abi arm64-v8a --platform android-21 --wolfssl-path ./install/wolfssl-android/arm64-v8a
```

**Or from plugin root:** `./build-native.sh --android-only --abi arm64-v8a` (builds WolfSSL → nghttp3 → ngtcp2 in one go).

**Optional (QuicTLS/OpenSSL):** Use `./build-openssl.sh ... --quictls` and `--openssl-path ./install/openssl-android --quictls` for ngtcp2 if not using WolfSSL.

### 2.3 Build TLS for Android (WolfSSL default / OpenSSL optional)
